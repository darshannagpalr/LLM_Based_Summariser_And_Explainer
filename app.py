...
# translate_client = translate.Client(api_key=GOOGLE_CLOUD_TRANSLATE_API_KEY)
...
def process_input():
    ...
    ocr_languages = request.form.getlist(
        "ocr_languages"
    )  # Get list of selected OCR languages
    ...
    if "text_input" in request.form and request.form["text_input"]:
        ...
        if image_file.filename != "":
            ...
            input_text = process_image_to_text(
                filepath, languages=tesseract_lang_string
            )
        ...
        return (
            jsonify({"error": "No valid input provided (text, voice, or image)."}),
            400,
        )
